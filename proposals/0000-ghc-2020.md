...
- _More derivable type classes_: `DeriveAnyClass`, `DeriveDataTypeable`, `DeriveFoldable`, `DeriveFunctor`, `DeriveGeneric`, `DeriveLift`, `DeriveTraversable`
- _Extended numeric literals_: `BinaryLiterals`, `HexFloatLiterals`, `NegativeLiterals`, `NumDecimals`, `NumericUnderscores`
- _Extensions to `deriving`_: `DeriveAnyClass`, `DerivingStrategies`, `DerivingVia`, `EmptyDataDeriving`, `GeneralisedNewtypeDeriving`, `StandaloneDeriving`
- `BangPatterns`
- `BlockArguments`
...
- `DataKinds`: the fact that you can use unticked constructors may lead to surprises. Maybe introduce a strict version?
...
- `MonadFailDesugaring`: this extension depends on the exhaustiveness checker right now, and there are proposals to change its behavior.
